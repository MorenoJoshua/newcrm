...
if(!$_GET['leadid']){echo 'no lead'; $check = '1';}
if(!$_GET['agent']){echo 'no agent'; $check = '1';}
if($check == '1'){echo 'There was an error'; die();}
...
print_r(error_get_last());
	...
	$cardiffpass = $passwordout;
		...
		// Create a message
		$message = Swift_Message::newInstance(sprintf('Bank of Cardiff Loan Application for %s', $app_amount))
		    ->setFrom(array($sender => $sender)) // can be $_POST['email'] etc...
		    ->setTo(array($reciever => $recievername)) // your email / multiple supported.
		    ->setBody(sprintf("WCEMAIL"), 'text/html')//;
			->attach($attachment);
		// Send the message
